...
def read_img(idx, base_path, set_name, version=None):
    ...
    # print(f"image path:{img_rgb_path}")
...
'''
show the image of the hand with keypoints+skeleton and hand mask
values: dictionary containing img, msk and uv (2D coordinated projected to image plane)
'''
def show_img_kpts(values):
    img=values['img']
    msk=values['msk']
    uv=values['uv']
